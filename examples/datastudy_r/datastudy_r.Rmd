...
births_df <- lemurs_smallts %>%
  ...
  dplyr::mutate_at(vars(birth_month), lubridate::month, label = TRUE) %>% ## id months
...
birth_season_countdf <- births_df %>%
  ...
  tidyr::pivot_longer(as.character(lubridate::month(1:12, label = TRUE)), 
                      names_to = "birth_month", values_to = "n_births")%>%
  ##dplyr::mutate_at(vars(birth_month), as.numeric) %>%
  dplyr::arrange(taxon, birth_month) 
  
  # unique() %>%
  # dplyr::mutate(birth = TRUE) %>%
  # tidyr::pivot_wider(id_cols = taxon, names_from = birth_month, values_from = birth) %>%
  # tidyr::pivot_longer(cols = 2:13, names_to = "birth_month", values_to = "birth")%>%
  # dplyr::mutate_at(vars(birth_month), as.numeric) %>%
  # dplyr::arrange(taxon, birth_month)
```
