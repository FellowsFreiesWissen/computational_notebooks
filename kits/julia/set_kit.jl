...
function set_kit(proj_name::String, proj_path::String)
    ...
    println(string("Will create folder ", joinpath(proj_path, proj_name), " for your project."))
    ...
    if user_ans == "no"
        ...
        if isdir(joinpath(proj_path, proj_name))
            println("There already is a folder with this name in this path. Move it or change its name.")
        else
            ...
            open(joinpath(proj_path, proj_name, "results/README.txt"), "w") do io
                ...
                write(io, "\n")
                write(io, "`figures`: figures resulting from the analysis of results.")
                write(io, "\n")
                write(io, "`tables`: tables resulting from the analysis of results.")
            end

            ### scripts folder
            touch(joinpath(proj_path, proj_name, "scripts/README.txt"))
            open(joinpath(proj_path, proj_name, "scripts/README.txt"), "w") do io
                write(io, "This folder contains all code used to process the data, and which, for some reason or another is not included in the notebook because they too cumbersome or not of upmost relevancy for comprehension.")
            end
            
            ### text folder
            touch(joinpath(proj_path, proj_name, "text/README.txt"))
            open(joinpath(proj_path, proj_name, "text/README.txt"), "w") do io
                write(io, "This folder contains the main text of the manuscript as well as a folder with the supplementary material and one with references.")
            end
            ...
            open(joinpath(proj_path, proj_name, string(proj_name, ".jl")), "w") do io
                ...
                write(io, "\n\n")
                write(io, "\tdata_dir = joinpath(\"results\", \"data\", \"processed\")  ## Do NOT play with stuff in data/raw. That is your back-up. Work only on `process`.")
                write(io, "\n")
                write(io, "\tscripts_dir = joinpath(\"scripts\")")
                write(io, "\n")
                write(io, "\tfigs_dir = joinpath(\"results\", \"figures\")")
                write(io, "\n")
                write(io, "\ttabs_dir = joinpath(\"results\", \"tables\")")
                write(io, "\n")
