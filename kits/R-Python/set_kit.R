...
set_kit <- function(proj_name, proj_path, lang = "r"){
  ...
  if(dir.exists(file.path(proj_path, proj_name))){
    print("There already is a project with this name in this path. Move it or change its name.")
  }else{
    ...
    sapply(file.path(paste(file.path(proj_path, proj_name, "text"), 
                           c("figures","tables", "supplementary", "references"),
                           sep = "/")), 
           dir.create)
    ...
    cat("\n")
    cat("`data/process`: these are files generated by processing the raw data and these are the ones used in the analysis. It can be a copy of the raw data if that is already ready to use. Any processing of the raw data to generate the files here should be documented in the notebook.")
    ...
    cat("This folder contains the main text of the manuscript, folders containing the figures and tables (unformatted) to be included in it, as well as a folder with the supplementary material and one to store references.")
    sink()
    ...
    cat("\n\n")
    cat("<!-- Text written like this, between \"<!-- -->!\" will be not be included in html file  -->")
    cat("\n\n")
    cat("This line includes `inline code`. Inline code is only formatted differently from narrative text, it is not executed.")
    cat("\n\n")
    cat("Blocks of code (also called 'chunks') are marked with backticks and the language between braces, like this:")
    cat("\n")
    cat(paste("```{", lang, "}", sep = ""))
    cat("\n")
    ...
    cat("\n")
    cat("figs_dir <- file.path(\"text\", \"figures\")")
    cat("\n")
    cat("tabs_dir <- file.path(\"text\", \"tables\")")
    cat("\n")
    cat("```")
    cat("\n\n")
    cat("# Read your data")
    ...
    cat("\n")
    cat("# To avoid the repetition of having the same figure here as well, either include a line that saves the figure in the figures directory, or include `eval=FALSE` in the chunk options, and the code will not run. In case you choose the latter, be aware that any objects created here will not actually be created when the notebook gets knitted. Only the code for it wll be shown.")
    cat("\n")
    cat("```")
    cat("\n\n")
    cat("## Figure S1")
